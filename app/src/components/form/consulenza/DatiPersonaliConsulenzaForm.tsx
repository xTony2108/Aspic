...
const formDatiPersonaliConsulenzaSchema = consulenzaSchema.pick({
  firstName: true,
  lastName: true,
  address: true,
  birthday: true,
...
type FormSchema = z.infer<typeof formDatiPersonaliConsulenzaSchema>;

export const DatiPersonaliConsulenzaForm = () => {
  const navigate = useNavigate();

  const appointmentDate = useConsulenzaFormStore((s) => s.appointmentDate);
  const appointmentTime = useConsulenzaFormStore((s) => s.appointmentTime);
  const urgent = useConsulenzaFormStore((s) => s.urgent);
  const clientAge = useConsulenzaFormStore((s) => s.clientAge);
  const clientType = useConsulenzaFormStore((s) => s.clientType);
  const reason = useConsulenzaFormStore((s) => s.reason);
  const firstName = useConsulenzaFormStore((s) => s.firstName);
  const lastName = useConsulenzaFormStore((s) => s.lastName);
  const address = useConsulenzaFormStore((s) => s.address);
  const birthday = useConsulenzaFormStore((s) => s.birthday);
  const birthPlace = useConsulenzaFormStore((s) => s.birthPlace);
  const fiscalCode = useConsulenzaFormStore((s) => s.fiscalCode);
  const phoneNumber = useConsulenzaFormStore((s) => s.phoneNumber);
  const email = useConsulenzaFormStore((s) => s.email);
  ...
  const {
    ...
    resolver: zodResolver(formDatiPersonaliConsulenzaSchema),
    defaultValues: {
      firstName: firstName || "",
      lastName: lastName || "",
      address: address || "",
