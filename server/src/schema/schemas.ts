...
export const verificationTokenSchema = z.object({
  token: z
    .string({ error: "Token non valido" })
    .trim()
    .min(1, "Token non valido")
    .regex(/^[a-f0-9]{64}$/i, "Token non valido"),
});

export const activateAccountSchema = verificationTokenSchema
  .extend({
    password: z
      .string({ error: "Inserisci la password" })
      .min(8, "La password deve contenere almeno 8 caratteri")
...
export type VerificationTokenTypeSchema = z.infer<typeof verificationTokenSchema>;
